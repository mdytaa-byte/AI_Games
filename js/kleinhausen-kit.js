...
(function (global) {
  ...
  KH.tex = {
    ...
    wasser: function () {
      return tex(function (g, s) {
        g.fillStyle = '#2e6a9a'; g.fillRect(0, 0, s, s);
        g.strokeStyle = 'rgba(220,240,255,.28)'; g.lineWidth = 2;
        for (var i = 0; i < 5; i++) {
          g.beginPath(); g.moveTo(0, 18 + i * 22);
          g.bezierCurveTo(32, 8 + i * 22, 80, 28 + i * 22, s, 16 + i * 22); g.stroke();
        }
        grain(g, s, 10);
      });
    },
    ...
    himmel: function () {
      return tex(function (g, s) {
        ...
        gr.addColorStop(0, '#8ec6e8'); gr.addColorStop(0.55, '#c5dff0'); gr.addColorStop(1, '#e8dcc0');
        g.fillStyle = gr; g.fillRect(0, 0, s, s);
        g.fillStyle = 'rgba(255,255,255,.55)';
        g.beginPath(); g.arc(40, 36, 18, 0, Math.PI * 2); g.arc(58, 38, 14, 0, Math.PI * 2); g.arc(28, 40, 12, 0, Math.PI * 2); g.fill();
      }, 128);
    }
  };
  ...
  World.prototype.pendant = function (x, z, y) {
    ...
    this.lamps.push({ x: x, y: y - 0.55, z: z, color: 0xffd9a0, int: 0.7, dist: 7 });
  };
  ...
  World.prototype.room = function (spec) {
    ...
    if (spec.ceiling !== false) this.box(w, 0.06, d, { x: x, y: h, z: z, color: spec.ceil || 0xf3ebe0, collide: false });
    var t = 0.18;
    ...
    if (spec.windows) {
      spec.windows.forEach(function (win) {
        ...
        self.plane(ww, hh, {
          ...
          color: 0xd4e8f6, emissive: 0x6688aa, transparent: true, opacity: 0.72
        });
  ...
  KH.start = function (cfg) {
    ...
    scene.add(new THREE.HemisphereLight(cfg.hemiTop || 0xfff1d6, cfg.hemiBot || 0x4a5a48, cfg.hemi || 0.72));
    var sun = new THREE.DirectionalLight(cfg.sun || 0xffe6c4, cfg.sunInt || 0.48);
    sun.position.set(6, 12, 5);
    scene.add(sun);
    scene.add(new THREE.AmbientLight(0xfff5ea, 0.18));
